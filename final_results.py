...
def calculate_size(K, D, V, ternary=False, ptq=False):
    ...
    if ternary and not ptq:
        return K * (
            (ternary_or_fp // 4) + (2 * norm_params_ternary) + (2 * scales_ternary)
        ) + (2 * (embeddings + D)), K * (ternary_or_fp + norm_params_ternary) + (
            embeddings + D
        )
    elif ptq:
        return K * (
            (ternary_or_fp // 4) + (2 * norm_params_fp) + (2 * scales_ternary)
        ) + (2 * (embeddings + D)), K * (
            ternary_or_fp + norm_params_fp + scales_ternary
        ) + (embeddings + D)
    else:
        return 2 * (K * (ternary_or_fp + norm_params_fp) + embeddings + D), (
            K * (ternary_or_fp + norm_params_fp) + embeddings + D
        )
...
if __name__ == "__main__":
    ...
    model_to_eval, info = load_model(
        checkpoint_path=args.model_path, quantize_to_ternary=args.ptq, return_info=True
    )
    ...
    size, num_params = calculate_size(
        info["K"], info["D"], config.VOCAB_SIZE, is_ternary, args.ptq
    )
