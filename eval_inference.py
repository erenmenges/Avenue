...
def predict(lm: model.Transformer, temperature: float = 0.8, max_tokens: int = 155, repetition_penalty: float = 1.2, top_k: int = 16384, top_p: float = 1.0, min_p: float = 0.12):
    ...
    with torch.no_grad():
        for _ in range(max_tokens - prompt_tokenized.shape[1]):
            ...
            probabilities = torch.where(probabilities >= threshold, probabilities, torch.zeros_like(probabilities))  ### min p sampling
            ...
            probabilities = torch.where(probabilities >= kth_largest_probability, probabilities, torch.zeros_like(probabilities))  ### top k sampling
...
if __name__ == "__main__":
    ...
    predict(lm=lm, temperature=args.temperature, top_k=args.top_k, top_p=args.top_p, min_p=args.min_p, repetition_penalty=args.repetition_penalty)
