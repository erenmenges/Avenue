...
def save_checkpoint(model: model.Transformer, model_config: dict, optimizers: tuple, step: int, muon_lr: float, adamw_lr: float, wandb_run_id: str, run_dir: Path, final: bool = False):
    savedict = {"model": model.state_dict(),
                "optimizers": {"muon": optimizers[0].state_dict(), "adamw": optimizers[1].state_dict()},
                "step": step,
                "config": model_config,
...
def build_optimizers(raw_lm: model.Transformer, muon_lr: float, adamw_lr: float):
    ...
    for name, p in raw_lm.named_parameters():
        if not p.requires_grad:
            continue
        is_embed = name.startswith("embeddings.") or name.startswith("output_head.")
    ...
    adamw_optimizer = torch.optim.AdamW(
        params=[{"params": decay_params, "weight_decay": 0.1},
...
def train(K: int, D: int, H: int, token_budget: int,muon_lr: float = config.MUON_LR, adamw_lr: float = config.ADAM_LR, seed: int = config.SEED, resume_path: str | None = None, is_ternary: bool = False):
    torch.manual_seed(seed)

    if not resume_path:
        raw_lm = model.Transformer(K=K, D=D, H=H, V=config.VOCAB_SIZE, ternary=is_ternary)
        raw_lm.to(device)
        model_config = {"K":K ,"D":D, "H":H, "V": config.VOCAB_SIZE, "MUON_LR": muon_lr, "ADAM_LR": adamw_lr,  "IS_TERNARY": is_ternary}
        reset_rngs("both")
        ...
        model_config = {"K":checkpoint_config["K"] ,"D":checkpoint_config["D"], "H":checkpoint_config["H"], "V": checkpoint_config["V"], "MUON_LR": checkpoint_config["MUON_LR"], "ADAM_LR": checkpoint_config["ADAM_LR"], "IS_TERNARY": checkpoint_config["IS_TERNARY"]}
    ...
    if not resume_path:
        ...
        run = wandb.init(project="Avenue",
                            name = (f"{architecture_type}_{n_params_in_millions:.0f}M_muon"
    ...
    for step in range(start_step, max_steps):
        ...
        if step % 300 == 0:
            ...
            wandb.log({
                        ...
                        step=step)
            print(f"Step: {step}, train loss: {loss.item():.3f}, val_loss: {val_loss:.3f}, time elapsed: {time.perf_counter() - start:.2f}s, tokens_trained: {tokens_trained}, tok/s:{(tokens_trained - session_start_tokens)/(time.perf_counter() - start):.2f}, grad_norm: {grad_norm.item():.2f}, MUON LR: {muon_optimizer.param_groups[0]["lr"]:.6f}, ADAMW LR: {adamw_optimizer.param_groups[0]["lr"]:.6f}")

        # save every 800 steps
        if step % 800 == 0 and step > start_step:
            save_checkpoint(raw_lm, model_config, (muon_optimizer, adamw_optimizer), step, muon_lr, adamw_lr, run.id, RUN_DIR)
    ...
    save_checkpoint(raw_lm, model_config, (muon_optimizer, adamw_optimizer), step, muon_lr, adamw_lr, run.id, RUN_DIR, True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--muon-lr", type=float, default=config.MUON_LR)
    parser.add_argument("--adamw-lr", type=float, default=config.ADAM_LR)
    parser.add_argument("--K", type=int, default=config.K)
    parser.add_argument("--D", type=int, default=config.D)
    parser.add_argument("--H", type=int, default=config.H)
    parser.add_argument("--token-budget", type=int, default=config.TRAINING_TOKEN_BUDGET)
    ...
    train(K=args.K, D=args.D, H=args.H, token_budget=args.token_budget, resume_path=args.resume, is_ternary=args.ternary, muon_lr=args.muon_lr, adamw_lr=args.adamw_lr)