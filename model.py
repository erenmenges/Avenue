...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        self.K_layer = make_linear(in_features=D, out_features=D)
        self.V_layer = make_linear(in_features=D, out_features=D)
        self.O_layer = make_linear(in_features=D, out_features=D)

        self.ln1 = make_norm()  ### identity if ternary, RMSNorm if FP
        self.ln2 = make_norm()  ### identity if ternary, RMSNorm if FP
        self.MLP = nn.Sequential(
            make_linear(in_features=D, out_features=4 * D),
            nn.GELU(),
            make_linear(in_features=4 * D, out_features=D),
        )
...
class Transformer(nn.Module):
    def __init__(self, K: int, D: int, H: int, V: int, ternary: bool = False):
        ...
        layers = []
        for _ in range(K):
            layers.append(TransformerBlock(D, H, self.RoPE, ternary))
        ...
        for name, p in self.named_parameters():
            if name.endswith(("O_layer.weight", "MLP.2.weight")):
